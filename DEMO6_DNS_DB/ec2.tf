...
data "aws_ami" "bastion_ami" {
  ...
  filter {
    name   = "name"
    values = ["PACKER-DEMO-ansible_image"]
  }
  ...
  */
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo mv /etc/ansible/ansible.cfg /etc/ansible/ansible.cfg-bck",
      "sudo mv /home/ubuntu/ansible/ansible.cfg /etc/ansible/ansible.cfg",
    ]
    on_failure = fail
  }
